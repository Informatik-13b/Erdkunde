  ...
  begin
     If Application.MessageBox('Wollen Sie wirklich die Anmeldung beenden?' +#10#13+
         ...
         = IDOK then begin     //Fragt den User, ob wirklich beendet werden soll!
                       ...
                       DeleteFile('KlassenNamen\' + EdtKlassenname.Text +'.txt');
  end;               end;
...
procedure TFormKlasseVerbinden.BtnBestaetigenClick(Sender: TObject);
 var Anzahl: Integer;
  begin
     ...
     StringGridKlassenNamen.Cells[0,PositionGrid] := EdtSchuelerNachname.Text;
     StringGridKlassenNAmen.Cells[1,PositionGrid] := EdtSchuelerVorname.Text;
     StringGridKlassenNAmen.Cells[2,PositionGrid] := Geschlecht;
     StringGridKlassenNamen.Cells[3,PositionGrid] := EdtKlassenName.Text;
     StringGridKlassenNamen.Cells[4,PositionGrid] := IntToStr(PositionGrid);
     ServerSocketAnmeldung.Socket.Connections[0].SendText (IntToStr(PositionGrid));  //Sendet den Index
...
procedure TFormKlasseVerbinden.SchreibeKlassenDatei();
 var i,k,L,z: Integer;
  begin
    For i := 0 to 8 do         //Alle 8 Spalten werden eingelesen
