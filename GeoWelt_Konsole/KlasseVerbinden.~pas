 ...
 var H: THandle;
  begin
     ...
     LabelAnweisung.Caption :=
      ('Achtung: Jeder anwesende Schüler muss sich einzeln anmelden!' +#10#13+
      ...
      +#10#13+ 
      '3. Lassen Sie einen weiteren Schüler sich anmelden.');  //Text wird dem Label
...
procedure TFormKlasseVerbinden.BtnCopyClick(Sender: TObject);
  begin
     showmessage('Lehrerkonsole V0.1' +#10#13+
                 ...
                 'OHG-Geesthacht-13b Entwicklerteam');    //Entwicklerinformationen
  end;
 ...
 var Anzahl: Integer;                    //Schüler wird eingepflegt
   begin
     ...
     If (Meldungen <= Anzahl) then
      begin
       BtnBestaetigen.Enabled := False;
       //showmessage (IntToStr(PositionGrid));
      ...
      If Anzahl = Meldungen then
        begin
          BtnBestaetigen.Caption := 'Beenden';
          BtnBestaetigen.Enabled := True;
        end;
     If (Meldungen = Anzahl +1) then
         begin
            ...
            FormKlasseAnlegen.Close();
            showmessage('Klasse angelegt!' +#10#13+
